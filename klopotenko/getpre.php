...
$sql='SELECT m.id,y.id AS id_site,y.site,y.paginator_link,y.paginator_img_s,y.paginator_short_text,y.paginator_short_text_del_link,m.rubrika,m.rubrika_name,m.category,m.category_name,m.page,m.page_end,m.max_page FROM sites_donor_options m
...
foreach($res as $k=>$v){
    ...
    for($y=1;$y<=$page;$y++){
        ...
        for($i=0;$i<$count;$i++){
            $sql='INSERT INTO sites_donor_link (id,id_opt,id_site,site,link_donor,img_s_donor,short_text_donor,rubrika,category) VALUES 
            (NULL,"'.$v['id'].'","'.$v['id_site'].'","'.$v['site'].'",'.$arrForDB['donor'][$i].','.$arrForDB['img_donor'][$i].','.$arrForDB['short_text_donor'][$i].','.'"'.$v['rubrika_name'].'","'.$v['category_name'].'");';
