...
foreach ($paginator as $link){
 // $a=pq($link)->html();	
  $catdir=pq($link)->attr('href'); 
  $mas_cat_url[]=$catdir;
  
  echo "<br>".date('H:i:s')." Открываем категорию ".$catdir;  
  $pagec=$pageText->get_page($catdir);
    ...
    $g=0;
     foreach ($prewviewgame as $pg){
      ...
      echo "<br>".date('H:i:s')." Получили страницу игры ".$url_game;  
         ...
         $catdir=str_replace("http://spanch-bob.org/", "", $catdir); 
         $path=realpath(__DIR__)."/content/".$catdir;
         
         $foldergame=str_replace(".html", "", basename($url_game)); 
         $path.="/".$foldergame;
         if (!file_exists($path)) mkdir($path, 0777, true);
         
         $path1=$path."/".$foldergame.basename($src);         
         file_put_contents ($path1,file_get_contents($src));
         ...
         $path2=$path."/".$foldergame.basename($filegame);         
         file_put_contents ($path2,file_get_contents($filegame));
